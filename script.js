...
const setGridDim = (dim, bypass) => {
	...
	// update buttons
	[document.getElementById('x11Btn'), document.getElementById('x9Btn'), document.getElementById('x7Btn')].forEach(
		(elem, index) =>
			(elem.src = index === 2 - (gridDim - 7) / 2 ? './icons/checked_true.svg' : './icons/checked_false.svg'),
	);
...
const init = () => {
	...
	snake = [[Math.floor(gridDim / 2), Math.floor(gridDim / 2)]];
	markSnake();
...
const markSnake = () =>
	snake.forEach((coords, index) => (state[coords[0]][coords[1]] = index == snake.length - 1 ? HEAD : SNAKE));

const spawnOrb = () => {
	let emptyList = [];
	state.forEach((row, rIndex) => {
		row.forEach((col, cIndex) => {
			if (col === EMPTY) emptyList.push([rIndex, cIndex]);
		});
	});
	...
	let id = setTimeout(() => {
		if (!pause) {
			if (nextFrame()) start();
				...
				// set highscore
				if (hScore < score) setHScore(score);
			}
		}
...
const updateCanvas = (newOrb, gameover) => {
	...
	// draw pos circle
	drawCircle(CIRCLE_RADIUS * 2, CIRCLE_RADIUS * 2, CIRCLE_RADIUS, ctx, COLOR_POS_CIRCLE);

	// draw direction circles
	CIRCLE_X.forEach((x, index) =>
		drawCircle(x, CIRCLE_RADIUS * 2, CIRCLE_RADIUS, ctx, snakeDir === index ? COLOR_LIGHT_GREEN : COLOR_DARK_GREEN),
	);

	// draw gameover circle
	if (gameover)
		drawCircle(canvas.width - CIRCLE_RADIUS * 2 - 20, CIRCLE_RADIUS * 2, CIRCLE_RADIUS, ctx, COLOR_LIGHT_GREEN);
	...
	for (let row = 0; row < gridDim; ++row) {
		for (let col = 0; col < gridDim; ++col) {
			...
			if (state[row][col] === HEAD) ctx.fillStyle = COLOR_DARK_GREEN;

			if (state[row][col] === SNAKE) ctx.fillStyle = COLOR_LIGHT_GREEN;

			if (state[row][col] === ORB) ctx.fillStyle = COLOR_LIGHT_RED;
...
document.addEventListener('keydown', (event) => {
	...
	// space bar
	if (event.key === ' ') {
		if (document.getElementById('state').innerHTML === 'GAMEOVER') init();
